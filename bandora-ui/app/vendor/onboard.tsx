  ...
  const handleNext = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/stores/onboard`, {
        ...
        body: JSON.stringify({ name, category, address, phone, opensAt, closesAt, isOpen }),
      });
      if (!res.ok) throw new Error('Failed to create store');
      const data = await res.json();
      router.push((`/vendor/services?storeId=${data.storeId}&storeName=${encodeURIComponent(name)}&category=${encodeURIComponent(category)}`) as any);
    } catch (e: any) {
      ...
      <View style={styles.footer}>
        ...
        >
          {loading ? (
            ...
            <>
              <Text style={styles.nextBtnText}>Next: Add Services</Text>
